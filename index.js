...
class Deprem {
  constructor(tarih, saat, enlem, boylam, derinlik, buyukluk, yer, sehir, id) {
    this.tarih = tarih;
    this.saat = saat;
    this.markerLat = enlem;
    this.markerLong = boylam;
    this.derinlik = derinlik;
    this.buyukluk = buyukluk;
    this.markerLabelText = yer;
    this.sehir = sehir;
  }
}
  ...
  request('http://www.koeri.boun.edu.tr/scripts/lst0.asp', (error, response, html) => {
    if (!error && response.statusCode == 200) {
      ...
      result.forEach((element) => {
        ...
        var tarih = depremBilgi[0];
        var saat = depremBilgi[1];
        var markerLat = depremBilgi[2];
        var markerLong = depremBilgi[3];
        var derinlik = depremBilgi[4];
        var buyukluk = depremBilgi[6];
        var markerLabelText = depremBilgi[8];
        var sehir = depremBilgi[9];
        if (sehir != null) {
...
app.get('/api/:id', (req,res) => {
  var depremler = getirDepremler();

  if(req.params.id != null){
    // depremler = depremler.filter((x) => x.id == req.params.id);
  }
